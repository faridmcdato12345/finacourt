...
@section('content')
    ...
    <section class="mx-auto max-w-3xl px-5 py-8 sm:px-8 sm:py-10">
        ...
        <form method="post" action="{{ route('player.preferences.update') }}" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" data-requires-online>
            ...
            <label class="mt-5 flex items-start gap-3 border-t border-slate-100 pt-5"><input type="checkbox" name="in_app_marketing_enabled" value="1" @checked($preference?->in_app_marketing_enabled) class="mt-1 size-5 rounded border-slate-300 text-court-700 focus:ring-court-500"><span><strong class="block text-slate-900">Show in-app messages</strong><span class="mt-1 block text-sm leading-6 text-slate-500">Comeback campaigns appear in your private booking updates.</span></span></label>
            <label class="mt-5 flex items-start gap-3 border-t border-slate-100 pt-5"><input type="checkbox" name="email_marketing_enabled" value="1" @checked($preference?->email_marketing_enabled) class="mt-1 size-5 rounded border-slate-300 text-court-700 focus:ring-court-500"><span><strong class="block text-slate-900">Email comeback messages</strong><span class="mt-1 block text-sm leading-6 text-slate-500">Court owners you previously booked with may email occasional comeback offers through FinACourt.</span></span></label>
