    ...
    <section class="mx-auto max-w-md px-5 py-12 sm:px-8 sm:py-20">
        <div data-player-card class="overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <div aria-hidden="true" class="-mx-6 -mt-6 mb-6 flex items-center justify-center gap-3 bg-court-950 px-6 py-4 text-court-200 sm:-mx-8 sm:-mt-8">
                <span class="size-2 rounded-full bg-court-300"></span><span class="text-xs font-semibold uppercase tracking-[0.18em]">Find · book · play</span><span class="size-2 rounded-full bg-amber-300"></span>
            </div>
            ...
            <form action="{{ route('player.login') }}" method="post" class="{{ ($socialProviders ?? []) === [] ? 'mt-7 ' : '' }}space-y-5">
                ...
                <div><div class="flex items-center justify-between gap-4"><label for="player-password" class="text-sm font-medium">Password</label><a href="{{ route('password.request', array_filter(['audience' => 'player', 'return' => request()->query('return')])) }}" class="text-sm font-semibold text-court-700 hover:text-court-800">Forgot password?</a></div><input id="player-password" name="password" type="password" autocomplete="current-password" required class="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3"></div>
