...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-[#f7f8f6] font-sans text-slate-950 antialiased">
        ...
        <header class="sticky top-0 z-30 border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
            <div class="page-shell flex min-h-16 items-center justify-between gap-3 py-2">
                <a href="{{ route('marketplace.home') }}" class="flex shrink-0 items-center gap-2.5 text-lg font-bold tracking-[0.1em] text-court-800 sm:text-xl">
                ...
                <nav class="flex min-w-0 items-center gap-1 text-sm font-medium" aria-label="Primary navigation">
                    <a href="{{ route('marketplace.courts.index') }}" aria-label="Find courts" class="flex items-center gap-2 whitespace-nowrap rounded-lg p-2 text-slate-700 hover:bg-slate-100 hover:text-court-800 sm:px-3">
                    ...
                    <a href="{{ route('marketplace.deals') }}" class="hidden whitespace-nowrap rounded-lg px-3 py-2 text-slate-700 hover:bg-slate-100 hover:text-court-800 md:block">Deals</a>
                    <a href="{{ route('marketplace.directory.index') }}" class="hidden whitespace-nowrap rounded-lg px-3 py-2 text-slate-700 hover:bg-slate-100 hover:text-court-800 lg:block">Venue guide</a>
                    @auth
                        <a href="{{ route('player.bookings.index') }}" class="hidden whitespace-nowrap rounded-lg px-3 py-2 text-slate-700 hover:bg-slate-100 sm:block">My bookings</a>
                        ...
                        @endif
                    @else
                        <a href="{{ route('marketplace.for-owners') }}" class="hidden whitespace-nowrap rounded-lg px-3 py-2 text-slate-700 hover:bg-slate-100 hover:text-court-800 lg:block">For owners</a>
                        <a href="{{ route('player.login') }}" class="hidden whitespace-nowrap rounded-lg px-3 py-2 text-slate-700 hover:bg-slate-100 sm:block">Log in</a>
                        <a href="{{ route('marketplace.for-owners') }}" class="whitespace-nowrap rounded-xl bg-court-700 px-3 py-2.5 font-semibold text-white shadow-sm hover:bg-court-800 sm:px-4"><span class="sm:hidden">Owner</span><span class="hidden sm:inline">List your courts</span></a>
                    @endauth
                </nav>
            </div>
        </header>
