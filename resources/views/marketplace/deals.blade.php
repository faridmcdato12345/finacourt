                ...
                @foreach ($promotions as $promotion)
                    ...
                    <article class="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-slate-900/8">
                        <div class="relative h-44 overflow-hidden bg-court-950 p-5 text-white">
                            ...
                            @endif
                            <div class="relative z-10 flex items-start justify-between gap-3"><span class="rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-court-800">{{ $promotion->offerLabel() ?: $promotion->promotion_type->label() }}</span><span class="rounded-lg bg-court-950/65 px-3 py-1.5 text-xs backdrop-blur">{{ $isUpcoming ? 'Starts '.$promotion->starts_on->format('M j') : 'Until '.$promotion->ends_on->format('M j') }}</span></div>
                        </div>
                        <div class="p-5"><p class="text-xs font-semibold uppercase tracking-wider text-court-700">{{ $promotion->venue->name }}</p><h2 class="mt-2 text-xl font-semibold">{{ $promotion->title }}</h2><p class="mt-2 text-sm text-slate-500">{{ $nextSlot?->resource?->name ?: $promotion->resource?->name ?: 'Eligible venue courts' }} · {{ $promotion->venue->publicCityName() }}</p><p class="mt-3 line-clamp-2 text-sm leading-6 text-slate-500">{{ $promotion->description ?: 'A published campaign from this venue.' }}</p><p class="mt-4 text-xs text-slate-400">@if ($nextSlot)Next promoted slot {{ $nextSlot->slot_date->format('M j, Y') }} · {{ substr($nextSlot->starts_at_time, 0, 5) }}–{{ substr($nextSlot->ends_at_time, 0, 5) }}@elseif ($isUpcoming)Available {{ $promotion->starts_on->format('M j, Y') }} through {{ $promotion->ends_on->format('M j, Y') }}@else Valid through {{ $promotion->ends_on->format('M j, Y') }}@if ($promotion->starts_at_time) · {{ substr($promotion->starts_at_time, 0, 5) }}–{{ substr($promotion->ends_at_time, 0, 5) }}@endif @endif</p><a href="{{ route('marketplace.venues.show', ['venueSlug' => $promotion->venue->slug, ...$promotion->marketplaceParameters()]) }}#availability" class="mt-5 block rounded-xl bg-court-700 px-5 py-3 text-center text-sm font-semibold text-white">{{ $isUpcoming ? 'Book for '.$promotion->starts_on->format('M j') : 'View eligible times' }}</a></div>
                    </article>
                @endforeach
            </div>
        @else
            <div class="app-card mt-8 px-6 py-16 text-center"><div class="mx-auto grid size-14 place-items-center rounded-2xl bg-court-50 text-2xl text-court-700">%</div><h2 class="mt-5 text-xl font-semibold">No published deals here</h2><p class="mt-2 text-sm text-slate-500">Browse regular published inventory while owners prepare their next campaigns.</p><a href="{{ route('marketplace.courts.index') }}" class="mt-5 inline-block rounded-xl bg-court-700 px-5 py-3 text-sm font-semibold text-white">Browse courts</a></div>
        @endif
    </section>
@endsection
