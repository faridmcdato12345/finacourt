            ...
            <div class="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                @foreach ($promotions as $promotion)
                    ...
                    <article class="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-slate-900/8">
                        ...
                        <div class="p-5"><p class="text-xs font-semibold uppercase tracking-wider text-court-700">{{ $promotion->venue->name }}</p><h2 class="mt-2 text-xl font-semibold">{{ $promotion->title }}</h2><p class="mt-2 text-sm text-slate-500">{{ $nextSlot?->resource?->name ?: $promotion->resource?->name ?: 'Eligible venue courts' }} · {{ $promotion->venue->city }}</p><p class="mt-3 line-clamp-2 text-sm leading-6 text-slate-500">{{ $promotion->description ?: 'A published campaign from this venue.' }}</p><p class="mt-4 text-xs text-slate-400">@if ($nextSlot)Next promoted slot {{ $nextSlot->slot_date->format('M j, Y') }} · {{ substr($nextSlot->starts_at_time, 0, 5) }}–{{ substr($nextSlot->ends_at_time, 0, 5) }}@elseif ($isUpcoming)Available {{ $promotion->starts_on->format('M j, Y') }} through {{ $promotion->ends_on->format('M j, Y') }}@else Valid through {{ $promotion->ends_on->format('M j, Y') }}@if ($promotion->starts_at_time) · {{ substr($promotion->starts_at_time, 0, 5) }}–{{ substr($promotion->ends_at_time, 0, 5) }}@endif @endif</p><a href="{{ route('marketplace.venues.show', ['venueSlug' => $promotion->venue->slug, ...$promotion->marketplaceParameters()]) }}#availability" class="mt-5 block rounded-xl bg-court-700 px-5 py-3 text-center text-sm font-semibold text-white">{{ $isUpcoming ? 'Book for '.$promotion->starts_on->format('M j') : 'View eligible times' }}</a></div>
                    </article>
                @endforeach
            </div>
        @else
