@php
    ...
    $promotionIsUpcoming = $promotion?->isUpcoming() ?? false;
...
<article class="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.03)] transition hover:-translate-y-0.5 hover:border-court-200 hover:shadow-xl hover:shadow-slate-900/8">
    <a href="{{ $venueUrl }}" class="block" aria-label="View {{ $venue->name }}">
        ...
        <div class="p-5">
            <div class="flex items-start justify-between gap-4"><div><h2 class="text-lg font-semibold tracking-tight text-slate-950 group-hover:text-court-800">{{ $venue->name }}</h2><p class="mt-1.5 flex items-center gap-1.5 text-sm text-slate-500">@include('marketplace.partials.icon', ['name' => 'location', 'class' => 'size-4 shrink-0 text-court-600']) {{ $venue->publicCityName() }}, {{ $venue->province }}</p>@if ($venue->getAttribute('distance_km') !== null)<p data-distance-km="{{ number_format((float) $venue->getAttribute('distance_km'), 2, '.', '') }}" class="mt-2 text-xs font-semibold text-court-700">{{ (float) $venue->getAttribute('distance_km') < 1 ? max(1, (int) round((float) $venue->getAttribute('distance_km') * 1000)).' m away' : number_format((float) $venue->getAttribute('distance_km'), 1).' km away' }}</p>@endif</div><span class="text-court-700 transition-transform group-hover:translate-x-1">@include('marketplace.partials.icon', ['name' => 'arrow-right', 'class' => 'size-5'])</span></div>
            @if ($venue->loyalty_active)
                <span class="mt-3 inline-flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-900"><span aria-hidden="true">★</span> Loyalty rewards</span>
            @endif
            @if ($promotion)<p class="mt-3 flex items-center gap-1.5 text-sm font-semibold text-amber-700">@include('marketplace.partials.icon', ['name' => 'tag', 'class' => 'size-4 shrink-0']) {{ $promotion->title }}@if ($promotionIsUpcoming)<span class="font-normal text-slate-500">· Starts {{ $promotion->starts_on->format('M j') }}</span>@endif</p>@endif
