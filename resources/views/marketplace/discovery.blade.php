        ...
        <div class="grid items-start gap-7 lg:grid-cols-[17rem_minmax(0,1fr)]">
            ...
            <div>
                ...
                @elseif ($cities->isNotEmpty() && $sports->isNotEmpty())
                    ...
                    @endphp
                    <section class="mt-12 border-t border-slate-200 pt-8"><h2 class="text-xl font-semibold">Explore local court guides</h2><div class="mt-4 flex flex-wrap gap-2">@foreach ($cities->take(6) as $city)<a href="{{ route('marketplace.courts.city', $city->city_slug) }}" class="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700">Sports Courts in {{ $city->publicCityName() }}</a>@endforeach @foreach ($guides as $guide)<a href="{{ route('marketplace.courts.sport-city', [$guide['sport']->slug, $guide['city_slug']]) }}" class="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700">{{ $guide['sport']->name }} Courts in {{ $guide['city'] }}</a>@endforeach</div></section>
                @endif
            </div>
        </div>
    </section>
