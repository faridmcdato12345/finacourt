...
async function checkAvailability() {
    ...
    try {
        ...
        if (!response.ok) throw new Error(Object.values(payload.errors || {}).flat()[0] || 'Availability could not be loaded.');
    ...
    <OwnerLayout>
        <div class="mx-auto max-w-6xl">
            ...
            <div class="mt-4">
                ...
                <p class="mt-2 text-sm text-slate-500">Manual, phone, Messenger, and walk-in bookings use the same conflict protection. Times use {{ timezone }}.</p>
            </div>

            <div v-if="!resources.length" class="mt-8 rounded-2xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-900">
                Add and activate a venue resource before creating bookings.
            </div>

            <form v-else class="mt-8 grid gap-6 lg:grid-cols-[1fr_23rem]" @submit.prevent="submit">
                <div class="space-y-6">
                    <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                        <h3 class="font-semibold text-slate-950">Resource and time</h3>
                        <div class="mt-5 grid gap-5 sm:grid-cols-2">
                            <label class="sm:col-span-2"><span class="text-sm font-medium text-slate-700">Resource</span><AppSelect v-model="form.resource_id" :options="resources.map((resource) => ({ value: resource.id, label: `${resource.venue} — ${resource.name} (${resource.sport})` }))" class="mt-2" aria-label="Booking resource" @change="resourceChanged" /><FormError :message="form.errors.resource_id" /></label>
                    ...
                    <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div class="mt-5 grid gap-5 sm:grid-cols-2">
                            ...
                            <label><span class="text-sm font-medium text-slate-700">Source</span><AppSelect v-model="form.source" :options="sources" class="mt-2" aria-label="Booking source" /><FormError :message="form.errors.source" /></label>
                            <label><span class="text-sm font-medium text-slate-700">State</span><AppSelect v-model="form.status" :options="statuses" class="mt-2" aria-label="Booking state" /><FormError :message="form.errors.status" /></label>
                ...
                <aside class="space-y-5">
                    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <h3 class="font-semibold text-slate-950">Availability</h3>
                        <p class="mt-2 text-xs leading-5 text-slate-500">This preview is advisory. The server checks again under a database lock when saving.</p>
                        ...
                        <div v-if="schedule" class="mt-4">
                            <p class="text-xs font-medium text-slate-500">{{ schedule.is_open ? `Open ${schedule.opens_at}–${schedule.closes_at}` : 'Closed or resource inactive' }}</p>
                    ...
                    <section v-if="selectedResource" class="rounded-2xl bg-slate-950 p-5 text-white">
                        <p class="text-xs uppercase tracking-wider text-slate-400">Base price snapshot</p>
                        ...
                        <p class="mt-2 text-xs leading-5 text-slate-400">The final total is calculated and stored by the server.</p>
                    </section>
