    ...
    <OwnerLayout>
        <div class="mx-auto max-w-6xl">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div><p class="text-sm font-semibold text-court-700">Past players</p><h2 class="mt-1 text-3xl font-semibold tracking-tight">Bring players back</h2><p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Send a one-time in-app or email message to players who booked with you before. FinACourt uses only the channels each player explicitly enabled.</p></div>
            ...
            <section class="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-6" aria-label="Past-player message results">
                <div v-for="metric in [
                    ['Players found', report.audience], ['Eligible', report.sent], ['Queued', report.delivered],
            ...
            <section class="mt-7">
                ...
                <div v-if="report.campaigns.length" class="mt-4 grid gap-4 md:grid-cols-2">
                    <Link v-for="campaign in report.campaigns" :key="campaign.id" :href="`/owner/reactivation/${campaign.id}`" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:border-court-300">
                        ...
                        <div class="mt-5 grid grid-cols-4 gap-2 border-t border-slate-100 pt-4 text-center text-xs"><div><strong class="block text-base">{{ campaign.audience }}</strong><span class="text-slate-400">Players</span></div><div><strong class="block text-base">{{ campaign.sent }}</strong><span class="text-slate-400">Eligible</span></div><div><strong class="block text-base">{{ campaign.delivered }}</strong><span class="text-slate-400">Queued</span></div><div><strong class="block text-base">{{ campaign.clicks }}</strong><span class="text-slate-400">Opened</span></div></div>
                    </Link>
                </div>
