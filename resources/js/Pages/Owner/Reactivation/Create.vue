...
<template>
    <Head title="Create comeback campaign" />
    <OwnerLayout>
        <div class="mx-auto max-w-3xl">
            <Link href="/owner/reactivation" class="text-sm font-semibold text-court-700">← Customer reactivation</Link>
            <h2 class="mt-5 text-3xl font-semibold tracking-tight">Create a comeback campaign</h2>
            <p class="mt-2 text-sm leading-6 text-slate-500">Nothing sends when you save. You will review the campaign and explicitly send it from the next page.</p>
            <form class="mt-7 space-y-6" @submit.prevent="submit">
                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                    ...
                    <div class="mt-5 grid gap-5 sm:grid-cols-2">
                        ...
                        <label class="block"><span class="text-sm font-medium">Segment</span><AppSelect v-model="form.segment" :options="segments" class="mt-2 min-h-12" /><FormError :message="form.errors.segment" /></label>
                        <label class="block sm:col-span-2"><span class="text-sm font-medium">Sport filter</span><AppSelect v-model="form.sport_id" :options="sports" option-value="id" option-label="name" class="mt-2 min-h-12" /><FormError :message="form.errors.sport_id" /></label>
                    </div>
                </section>
                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                    ...
                    <p class="mt-2 text-xs text-slate-400">In-app only. The platform does not invent email, SMS, or push-provider delivery.</p>
                </section>
