...
function send() {
    if (window.confirm('Send this one-time in-app message to past players who agreed to receive messages?')) router.post(`/owner/reactivation/${props.campaign.id}/send`);
}
...
<template>
    ...
    <OwnerLayout>
        <div class="mx-auto max-w-4xl">
            ...
            <section v-if="campaign.status === 'sent'" class="mt-7 grid gap-3 sm:grid-cols-5"><div v-for="metric in [['Players found', campaign.audience], ['Sent', campaign.sent], ['Reached', campaign.delivered], ['Not sent', campaign.suppressed], ['Opened link', campaign.clicks]]" :key="metric[0]" class="rounded-2xl border border-slate-200 bg-white p-4 text-center shadow-sm"><strong class="text-2xl">{{ metric[1] }}</strong><p class="mt-1 text-xs text-slate-400">{{ metric[0] }}</p></div></section>
        </div>
    </OwnerLayout>
</template>
