<script setup>
import { Head, Link, router, useForm } from '@inertiajs/vue3';
import VenueForm from '../../../Components/VenueForm.vue';
import VenuePhotoManager from '../../../Components/VenuePhotoManager.vue';
import GoogleBusinessProfilePanel from '../../../Components/GoogleBusinessProfilePanel.vue';
import OwnerLayout from '../../../Layouts/OwnerLayout.vue';

const props = defineProps({ venue: Object, sports: Array, amenities: Array, locationParents: Array, mapTileUrl: String, googleBusinessProfile: Object, returnToOnboarding: Boolean });

const form = useForm({
    name: props.venue.name,
    slug: props.venue.slug,
    description: props.venue.description || '',
    address: props.venue.address,
    city: props.venue.city,
    province: props.venue.province,
    psgc_parent_code: props.venue.psgc_parent_code || '',
    psgc_city_municipality_code: props.venue.psgc_city_municipality_code || '',
    latitude: props.venue.latitude || '',
    longitude: props.venue.longitude || '',
    phone: props.venue.phone || '',
    email: props.venue.email || '',
    website: props.venue.website || '',
    is_published: props.venue.is_published,
    sports: props.venue.sports,
    amenities: props.venue.amenities,
    onboarding: props.returnToOnboarding,
});
function submit() {
    form.put(`/owner/venues/${props.venue.id}`);
}

function destroyVenue() {
    if (window.confirm(`Delete ${props.venue.name}, its courts, photos, and opening hours? This cannot be undone.`)) {
        router.delete(`/owner/venues/${props.venue.id}`);
    }
}

</script>

<template>
    <Head :title="`Edit ${venue.name}`" />
    <OwnerLayout>
        <div class="mx-auto max-w-5xl">
            <Link :href="returnToOnboarding ? '/owner/onboarding/venue' : `/owner/venues/${venue.id}`" class="text-sm font-semibold text-court-700 hover:text-court-800">← {{ returnToOnboarding ? 'Back to setup progress' : 'Back to venue' }}</Link>
            <h2 class="mt-4 text-3xl font-semibold tracking-tight text-slate-950">Edit {{ venue.name }}</h2>
            <p class="mt-2 text-slate-600">Update photos, location, sports, facilities, contact details, and whether players can see it.</p>
            <div class="mt-8 space-y-7">
                <VenuePhotoManager :venue="venue" />
                <VenueForm :form="form" :sports="sports" :amenities="amenities" :location-parents="locationParents" :existing-state="venue" :map-tile-url="mapTileUrl" submit-label="Save venue" @submit="submit" />
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div><p class="text-xs font-semibold uppercase tracking-wider text-court-700">Repeat players</p><h3 class="mt-2 text-xl font-semibold">Venue loyalty</h3><p class="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Loyalty has its own page for reward settings, activation, and stamp results.</p></div>
                        <span :class="['rounded-full px-3 py-1 text-sm font-semibold', venue.loyalty_active ? 'bg-court-50 text-court-800' : 'bg-slate-100 text-slate-600']">{{ venue.loyalty_active ? 'Active' : 'Inactive' }}</span>
                    </div>
                    <p class="mt-4 text-sm text-slate-600">Current terms: {{ venue.loyalty_stamps_required }} stamps for {{ venue.loyalty_discount_percent }}% off, capped at ₱{{ venue.loyalty_discount_cap }}.</p>
                    <Link v-if="venue.can_manage_loyalty" :href="`/owner/loyalty?venue=${venue.id}`" class="mt-5 inline-flex rounded-xl bg-court-700 px-4 py-2.5 text-sm font-semibold text-white">Manage loyalty →</Link>
                    <p v-else class="mt-4 text-xs text-slate-500">Only the organization owner can manage loyalty.</p>
                </section>
                <GoogleBusinessProfilePanel :profile="googleBusinessProfile" />
            </div>

            <section class="mt-10 rounded-2xl border border-red-200 bg-red-50 p-6">
                <h3 class="font-semibold text-red-900">Delete venue</h3>
                <p class="mt-1 text-sm leading-6 text-red-700">This also deletes its courts, opening hours, selected facilities, and uploaded photos.</p>
                <button type="button" class="mt-4 rounded-xl border border-red-300 bg-white px-4 py-2.5 text-sm font-semibold text-red-700 hover:bg-red-100" @click="destroyVenue">Delete venue</button>
            </section>
        </div>
    </OwnerLayout>
</template>
