        ...
        <div class="mx-auto max-w-6xl">
            ...
            <div class="mt-4 flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div>
                    ...
                    <p v-if="venue.requires_platform_review && !venue.verified_at" class="mt-3 max-w-2xl rounded-xl bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">{{ venue.marketplace_review_requested_at ? 'Your final review request was sent to FinACourt. This venue stays private until the check is complete. We’ll email you when it is visible to players.' : 'This venue stays private until FinACourt completes its final check. Finish the venue details, add a court players can book, then select “Show this venue to players” and save to request the check.' }}</p>
                </div>
