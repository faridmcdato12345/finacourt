...
const metrics = () => [
    { label: 'Venue views', value: props.marketplace.profile_views, detail: 'Marketplace profile visits', icon: 'eye' },
    { label: 'Booking starts', value: props.marketplace.booking_starts, detail: 'Last 30 days', icon: 'calendar' },
    { label: 'Completed bookings', value: props.marketplace.completed_bookings, detail: `${props.marketplace.conversion_rate}% profile conversion`, icon: 'check' },
    { label: 'Booking revenue', value: money(props.marketplace.booking_revenue), detail: 'Qualified booking value', icon: 'wallet' },
    { label: 'New customers', value: props.marketplace.new_customers, detail: 'First confirmed booking', icon: 'users' },
];
</script>
    ...
    <OwnerLayout>
        <div class="mx-auto max-w-[92rem] space-y-7">
            <div class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                <div><p class="eyebrow">Overview</p><h1 class="mt-2 text-3xl font-semibold tracking-[-0.035em] text-slate-950 sm:text-4xl">Dashboard</h1><p class="mt-2 text-sm text-slate-500">What is happening across {{ organization.name }} today.</p></div>
            ...
            <section class="app-card overflow-hidden">
                <div class="flex flex-col gap-3 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><p class="eyebrow">Evidence into action</p><h3 class="mt-1 text-xl font-semibold">Growth opportunities</h3><p class="mt-2 text-sm text-slate-500">Prioritized, deterministic suggestions from current marketplace and booking facts.</p></div><Link href="/owner/growth" class="text-sm font-semibold text-court-700">View all →</Link></div>
                <div v-if="growth.active.length" class="grid gap-px bg-slate-100 lg:grid-cols-3"><div v-for="recommendation in growth.active" :key="recommendation.key" class="bg-white p-5 sm:p-6"><div class="flex items-center gap-2"><span :class="recommendation.priority === 'high' ? 'bg-amber-50 text-amber-800' : 'bg-court-50 text-court-800'" class="rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider">{{ recommendation.priority }}</span><span class="text-xs text-slate-400">{{ recommendation.venue || recommendation.type_label }}</span></div><h4 class="mt-4 font-semibold leading-6 text-slate-900">{{ recommendation.title }}</h4><p class="mt-2 line-clamp-3 text-sm leading-6 text-slate-500">{{ recommendation.explanation }}</p><Link :href="recommendation.suggested_action.url" class="mt-4 inline-block text-sm font-semibold text-court-700">{{ recommendation.suggested_action.label }} →</Link></div></div>
                <div v-else class="px-6 py-12 text-center"><p class="font-semibold text-slate-800">No supported recommendation yet</p><p class="mt-2 text-sm text-slate-500">The engine waits for enough real evidence instead of inventing an opportunity.</p></div>
            </section>

            <div class="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
                <section class="app-card overflow-hidden"><div class="flex flex-col gap-3 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><p class="eyebrow">Daily operations</p><h3 class="mt-1 text-xl font-semibold">Today’s schedule</h3></div><div class="flex gap-2"><span class="rounded-full bg-court-50 px-3 py-1.5 text-xs font-semibold text-court-800">{{ today.bookings }} bookings</span><span class="rounded-full bg-amber-50 px-3 py-1.5 text-xs font-semibold text-amber-800">{{ today.pending_payments }} pending payments</span></div></div>
