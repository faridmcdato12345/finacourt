...
const props = defineProps({ venue: Object, sports: Array, resourceTypes: Array, settings: Array, increments: Array, returnToOnboarding: Boolean });
const form = useForm({
...
<template>
    ...
    <OwnerLayout>
        <div class="mx-auto max-w-4xl">
            <Link :href="returnToOnboarding ? '/owner/onboarding/venue' : `/owner/venues/${venue.id}`" class="text-sm font-semibold text-court-700">← {{ returnToOnboarding ? 'Back to setup progress' : `Back to ${venue.name}` }}</Link>
            <h2 class="mt-4 text-3xl font-semibold tracking-tight text-slate-950">Add a court</h2>
            <p class="mt-2 text-slate-600">Choose the sport, whether the court is indoors or outdoors, the shortest booking time, and the hourly price.</p>
