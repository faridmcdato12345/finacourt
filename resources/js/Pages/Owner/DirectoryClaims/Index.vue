...
<template>
    ...
    <OwnerLayout>
        <div class="mx-auto max-w-5xl space-y-7">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="eyebrow">Bring your venue to FinACourt</p><h1 class="mt-2 text-3xl font-semibold tracking-tight">Ownership reviews</h1><p class="mt-3 max-w-2xl text-sm leading-6 text-slate-600">A request never gives instant control. We confirm access through an existing venue contact or an independent offline check, allow a safety hold, and review marketplace readiness separately.</p></div><a href="/directory" class="rounded-xl bg-court-700 px-4 py-3 text-sm font-semibold text-white">Find my venue</a></div>
            ...
            <section class="space-y-4">
                <article v-for="claim in claims" :key="claim.id" class="app-card p-5 sm:p-6">
                    ...
                    <div v-if="claim.status === 'pending'" class="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <div v-if="claim.proof_status === 'verified'" class="text-sm leading-6 text-court-900"><strong>Ownership proof confirmed.</strong> Approval remains unavailable until {{ claim.approval_available_at }} so unauthorized requests can be reported.</div>
                        <p v-else-if="claim.proof_status === 'locked'" class="text-sm leading-6 text-slate-600"><strong>Email-code attempts are locked.</strong> FinACourt must now confirm ownership through an independent offline check.</p>
                        <div v-else-if="claim.can_request_email_code">
                            <p class="text-sm leading-6 text-slate-600">Enter the six-digit code sent to {{ claim.proof_destination || 'the venue’s public email' }}. This proves access to an independently sourced venue contact.</p>
                        ...
                        <p v-else class="text-sm leading-6 text-slate-600">The directory has no public venue email. FinACourt must independently verify an official phone number, venue-domain email, business documents, or an in-person visit. Your submitted contact is not treated as proof.</p>
                    </div>
                </article>
                <div v-if="!claims.length" class="app-card px-6 py-14 text-center"><h2 class="text-lg font-semibold">No requests yet</h2><p class="mt-2 text-sm text-slate-500">Browse the local venue guide, open your venue, then choose “Request ownership review.”</p></div>
            </section>
        </div>
    </OwnerLayout>
</template>
