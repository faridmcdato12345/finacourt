            ...
            <section class="space-y-4">
                <article v-for="claim in claims" :key="claim.id" class="app-card p-5 sm:p-6">
                    ...
                    <div v-if="claim.status === 'pending'" class="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        <p v-else-if="claim.proof_status === 'locked'" class="text-sm leading-6 text-slate-600"><strong>Too many code tries.</strong> FinACourt must now check your connection to the venue another safe way.</p>
                        <div v-else-if="claim.can_request_email_code">
                            <p class="text-sm leading-6 text-slate-600">Enter the six-digit code sent to {{ claim.proof_destination || 'the venue’s public email' }}. This shows that you can access a venue contact FinACourt already found.</p>
                        ...
                        <p v-else class="text-sm leading-6 text-slate-600">This listing has no public venue email. FinACourt must check an official phone number, venue email, business document, or in-person visit. The contact you entered helps us review the request, but it is not enough on its own.</p>
                    </div>
                </article>
