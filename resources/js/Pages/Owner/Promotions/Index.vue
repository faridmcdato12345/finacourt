...
<template>
    ...
    <OwnerLayout>
        <div class="mx-auto max-w-6xl">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-sm font-semibold text-court-700">Empty-slot recovery</p><h2 class="mt-1 text-3xl font-semibold tracking-tight">Promotion campaigns</h2><p class="mt-2 text-sm text-slate-500">Turn real unsold court inventory into focused marketplace campaigns.</p></div><Link href="/owner/promotions/create" class="rounded-xl bg-court-700 px-4 py-2.5 text-center text-sm font-semibold text-white">Create campaign</Link></div>
            ...
            <div v-if="promotions.length" class="mt-7 grid gap-4 md:grid-cols-2">
                <Link v-for="promotion in promotions" :key="promotion.id" :href="`/owner/promotions/${promotion.id}`" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:border-court-300">
                    ...
                    <p class="mt-4 text-sm text-slate-600">{{ promotion.offer_label || 'Promotional placement' }} · {{ promotion.starts_on }} to {{ promotion.ends_on }}<span v-if="promotion.slots_count"> · {{ promotion.slots_count }} targeted slots</span></p>
                    <div class="mt-5 grid grid-cols-4 gap-2 border-t border-slate-100 pt-4 text-center text-xs"><div><strong class="block text-base text-slate-900">{{ promotion.impressions_count }}</strong><span class="text-slate-400">Views</span></div><div><strong class="block text-base text-slate-900">{{ promotion.clicks_count }}</strong><span class="text-slate-400">Clicks</span></div><div><strong class="block text-base text-slate-900">{{ promotion.booking_starts_count }}</strong><span class="text-slate-400">Starts</span></div><div><strong class="block text-base text-slate-900">{{ promotion.bookings_count }}</strong><span class="text-slate-400">Bookings</span></div></div>
                </Link>
            </div>
            <div v-else class="mt-7 rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center"><h3 class="font-semibold">No promotions yet</h3><p class="mt-2 text-sm text-slate-500">Create a focused campaign for real published inventory.</p></div>
