...
<template>
    <Head title="Help players find you" />
    <OwnerLayout>
        <div class="mx-auto max-w-[92rem] space-y-7">
            <header class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    ...
                    <h1 class="mt-2 text-3xl font-semibold tracking-[-0.035em] text-slate-950 sm:text-4xl">Help players find you</h1>
                    <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Improve your public page, share booking QR codes, and see what still needs to be filled in. Google setup is optional.</p>
                </div>
                <div class="app-card grid gap-2 p-4 text-xs text-slate-600 sm:grid-cols-2">
                    <div class="rounded-xl bg-slate-50 px-3 py-2.5"><strong class="block text-slate-900">Address search</strong>{{ integrations.places.label }}</div>
                    <div class="rounded-xl bg-slate-50 px-3 py-2.5"><strong class="block text-slate-900">Google listing</strong>{{ integrations.business_profile.label }}</div>
                </div>
            </header>

            <div class="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm leading-6 text-amber-950">
                <strong>About this check:</strong> {{ scoreNote }}
            </div>
            ...
            <div v-if="venues.length === 0" class="app-card p-10 text-center">
                ...
                <p class="mt-2 text-sm text-slate-500">Your public-page checklist will appear after your first venue is saved.</p>
                ...
                <div class="grid gap-6 border-b border-slate-200 bg-[linear-gradient(120deg,#f0fbf5_0%,#ffffff_62%)] p-5 sm:p-7 lg:grid-cols-[1fr_auto] lg:items-center">
                    <div>
                        ...
                        <div class="mt-4 flex flex-wrap gap-2 text-xs font-semibold">
                            <span class="rounded-full bg-court-50 px-3 py-1.5 text-court-800">FinACourt page: {{ venue.marketplace_status }}</span>
                            <span class="rounded-full bg-slate-100 px-3 py-1.5 text-slate-700">Search page: {{ venue.seo_status }}</span>
                ...
                <div class="grid gap-7 p-5 sm:p-7 xl:grid-cols-[1.15fr_.85fr]">
                    <section>
                        <div class="flex items-end justify-between gap-4">
                            <div><p class="eyebrow">Public page checklist</p><h3 class="mt-1 text-xl font-semibold text-slate-950">Make your venue page useful to players</h3></div>
                        ...
                        <div class="mt-5 grid gap-3 sm:grid-cols-2">
                            <div v-for="check in venue.checks" :key="check.code" :class="['rounded-2xl border p-4', check.complete ? 'border-court-100 bg-court-50/60' : 'border-slate-200 bg-white']">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div><h4 class="text-sm font-semibold text-slate-900">{{ check.label }}</h4><p class="mt-1 text-xs leading-5 text-slate-500">{{ check.complete ? 'Done' : check.guidance }}</p></div>
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="space-y-5">
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                            ...
                            <div v-if="venue.public_url" class="mt-4 space-y-3">
                                <div v-for="item in [
                                    ...
                                    { label: 'Booking link for Google', value: venue.google_booking_url, key: `google-${venue.id}` },
                                ]" :key="item.key">
                        ...
                        <div class="rounded-2xl border border-slate-200 p-5">
                            ...
                            <p class="mt-2 text-xs leading-5 text-slate-500">Each scan opens a live FinACourt page. Players still have to choose an available time before booking.</p>
