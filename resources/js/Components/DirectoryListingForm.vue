...
const validationErrors = computed(() => [...new Set(Object.values(form.errors).filter(Boolean))]);
...
function submit() {
    const options = {
        onError: () => nextTick(() => {
            ...
            errorSummary.value?.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }),
    };

    if (props.listing) form.put(`/platform/directory/${props.listing.slug}`, options);
    else form.post('/platform/directory', options);
}
</script>
...
<template>
    <form class="space-y-6" @submit.prevent="submit">
        <div
            ...
            tabindex="-1"
            role="alert"
            class="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800 outline-none focus:ring-2 focus:ring-red-300"
        >
            <p class="font-semibold">The venue was not saved. Please correct the following:</p>
        ...
        <section class="app-card grid gap-5 p-6 sm:grid-cols-2"><div class="sm:col-span-2"><h2 class="text-lg font-semibold">Venue details</h2><p class="mt-1 text-sm text-slate-500">Use your own wording and include only details that anyone can check publicly. Leave out reviews, photos, prices, and availability until the owner joins.</p></div>
            ...
            <label class="text-sm font-semibold sm:col-span-2">Short description <span class="text-slate-400">(optional)</span><textarea v-model="form.description" maxlength="3000" rows="4" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal" placeholder="Write a short factual description in your own words."></textarea><span v-if="form.errors.description" class="mt-1 block text-xs text-red-600">{{ form.errors.description }}</span></label>
            ...
            <template v-if="usesPsgcCatalog">
                <div>
                    ...
                    <SearchableSelect id="directory_psgc_parent_code" v-model="form.psgc_parent_code" :options="locationParents" option-value="code" option-label="label" placeholder="Search for a province or region" search-label="Province or region" empty-label="No matching province or region found." required autocomplete="address-level1" aria-label="Search for a province or region" class="mt-2" />
                ...
                <div>
                    ...
                    <SearchableSelect id="directory_psgc_city_code" v-model="form.psgc_city_municipality_code" :options="cityMunicipalities" option-value="code" option-label="name" :placeholder="locationOptionsLoading ? 'Loading locations…' : form.psgc_parent_code ? 'Search for a city or municipality' : 'Select a province or region first'" search-label="City or municipality" empty-label="No matching city or municipality found." required autocomplete="address-level2" :disabled="!form.psgc_parent_code || locationOptionsLoading" aria-label="Search for a city or municipality" class="mt-2" />
            ...
            <label class="flex items-start gap-3 rounded-xl bg-slate-50 p-4 text-sm sm:col-span-2"><input v-model="form.coordinates_verified" type="checkbox" class="mt-1 size-4 accent-court-700"><span><strong class="block">I checked that this map pin points to the venue</strong><span class="mt-1 block text-slate-500">If it is not checked, we will ask the owner to confirm the location later.</span><span v-if="form.errors.coordinates_verified" class="mt-1 block text-xs text-red-600">{{ form.errors.coordinates_verified }}</span></span></label>
            <label class="text-sm font-semibold">Public phone<input v-model="form.phone" maxlength="40" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal"><span v-if="form.errors.phone" class="mt-1 block text-xs text-red-600">{{ form.errors.phone }}</span></label><label class="text-sm font-semibold">Public email<input v-model="form.email" type="email" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal"><span v-if="form.errors.email" class="mt-1 block text-xs text-red-600">{{ form.errors.email }}</span></label><label class="text-sm font-semibold sm:col-span-2">Public website<input v-model="form.website" type="url" placeholder="https://example.com" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal"><span v-if="form.errors.website" class="mt-1 block text-xs text-red-600">{{ form.errors.website }}</span></label>
        </section>
        ...
        <section class="app-card p-6"><h2 class="text-lg font-semibold">Opening hours</h2><p class="mt-1 text-sm text-slate-500">Only add hours you found in a trustworthy public source. Players will still be asked to confirm them with the venue.</p><div class="mt-5 space-y-3"><div v-for="(hour, index) in form.hours" :key="hour.day_of_week" class="grid items-center gap-3 rounded-xl border border-slate-200 p-3 sm:grid-cols-[130px_110px_1fr_1fr]"><span class="text-sm font-semibold">{{ weekdays.find((day) => Number(day.value) === Number(hour.day_of_week))?.label }}</span><label class="flex items-center gap-2 text-sm"><input v-model="hour.is_closed" type="checkbox" class="size-4 accent-court-700">Closed</label><input v-model="hour.opens_at" :disabled="hour.is_closed" type="time" class="rounded-lg border border-slate-200 px-3 py-2 disabled:bg-slate-100"><input v-model="hour.closes_at" :disabled="hour.is_closed" type="time" class="rounded-lg border border-slate-200 px-3 py-2 disabled:bg-slate-100"><div v-if="form.errors[`hours.${index}.day_of_week`] || form.errors[`hours.${index}.is_closed`] || form.errors[`hours.${index}.opens_at`] || form.errors[`hours.${index}.closes_at`]" class="space-y-1 text-xs text-red-600 sm:col-span-4"><p v-if="form.errors[`hours.${index}.day_of_week`]">{{ form.errors[`hours.${index}.day_of_week`] }}</p><p v-if="form.errors[`hours.${index}.is_closed`]">{{ form.errors[`hours.${index}.is_closed`] }}</p><p v-if="form.errors[`hours.${index}.opens_at`]">{{ form.errors[`hours.${index}.opens_at`] }}</p><p v-if="form.errors[`hours.${index}.closes_at`]">{{ form.errors[`hours.${index}.closes_at`] }}</p></div></div></div><span v-if="form.errors.hours" class="mt-2 block text-xs text-red-600">{{ form.errors.hours }}</span></section>

        <section class="app-card grid gap-5 p-6 sm:grid-cols-2"><div class="sm:col-span-2"><h2 class="text-lg font-semibold">Where did the information come from?</h2><p class="mt-1 text-sm text-slate-500">Add enough detail for another team member to check it. At least one source link or private source note is required. We keep a private record of changes, but only the public source type and link appear on the venue page.</p></div><label class="text-sm font-semibold">Type of source<AppSelect v-model="form.source_type" :options="sourceTypes" class="mt-2" /><span v-if="form.errors.source_type" class="mt-1 block text-xs text-red-600">{{ form.errors.source_type }}</span></label><div></div><label class="text-sm font-semibold sm:col-span-2">Public source link <span class="text-slate-400">(or add a note below)</span><input v-model="form.source_url" type="url" placeholder="https://example.com/venue-details" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal"><span v-if="form.errors.source_url" class="mt-1 block text-xs text-red-600">{{ form.errors.source_url }}</span></label><label class="text-sm font-semibold sm:col-span-2">Private source note <span class="text-slate-400">(required when no public link is available)</span><input v-model="form.source_reference" maxlength="500" class="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal" placeholder="For example: checked during a venue visit or against a registry record"><span v-if="form.errors.source_reference" class="mt-1 block text-xs text-red-600">{{ form.errors.source_reference }}</span></label>
        ...
        <button :disabled="form.processing" class="rounded-xl bg-court-700 px-6 py-3 text-sm font-semibold text-white disabled:opacity-50">{{ form.processing ? 'Saving…' : (listing ? 'Save changes' : 'Save as draft') }}</button>
    </form>
</template>
