...
<template>
    <form class="space-y-7" @submit.prevent="$emit('submit')">
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="mb-6">
                <h2 class="text-lg font-semibold text-slate-950">Venue identity</h2>
                <p class="mt-1 text-sm text-slate-500">The public-facing name and description for this location.</p>
            </div>
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    <label for="slug" class="mb-2 block text-sm font-medium text-slate-800">URL slug <span class="font-normal text-slate-400">(generated if blank)</span></label>
        ...
        <section v-if="allowPhotoUpload" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div>
                ...
                <p class="mt-1 text-sm leading-6 text-slate-500">Add initial photos now. The first selected image becomes the public cover photo.</p>
            </div>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-slate-500">City and province are indexed for future marketplace filtering.</p>
            </div>
            <div class="grid gap-5 sm:grid-cols-2">
                <div class="sm:col-span-2">
                ...
                <div class="sm:col-span-2">
                    <p class="mb-3 text-xs leading-5 text-slate-500">Enter both coordinates and confirm that the pin identifies the venue entrance. Saving marks the pin as owner-verified.</p>
                    ...
                    <p v-else class="rounded-xl bg-slate-50 px-4 py-5 text-sm text-slate-500">The map preview appears after both latitude and longitude are entered.</p>
                </div>
            </div>
        </section>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="grid gap-8 lg:grid-cols-2">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">At least one sport is required. Resources can only use selected sports.</p>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm leading-6 text-slate-500">{{ existingState?.requires_platform_review && !existingState?.is_verified ? 'This records your publication request. Because this venue came from an ownership claim, it remains blocked from the marketplace until FinACourt completes a separate venue and court review.' : 'Published venues with at least one active resource and sport are eligible for the public marketplace.' }}</p>
            ...
            <div v-if="existingState" class="mt-5 flex flex-wrap gap-2 border-t border-slate-100 pt-5 text-xs font-semibold">
                ...
                <span class="rounded-full bg-slate-100 px-3 py-1.5 text-slate-600">{{ existingState.is_verified ? 'Verified' : 'Verification pending' }}</span>
            </div>
        </section>
