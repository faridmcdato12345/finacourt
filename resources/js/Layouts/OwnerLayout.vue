            ...
            <div v-show="mobileNavigationOpen" id="owner-mobile-menu" class="border-t border-white/10 px-3 pb-4 pt-3">
                ...
                <nav class="owner-mobile-navigation filter-scrollbar grid max-h-[calc(100dvh-10.5rem)] gap-1 overflow-y-auto pr-1 text-sm sm:grid-cols-2" aria-label="Mobile owner navigation" @click="closeMobileNavigation">
                    ...
                    <template v-if="isEmailVerified && !isWorkspaceRestricted">
                        ...
                        <Link v-if="abilities.manage_loyalty" href="/owner/loyalty" :class="['rounded-xl px-4 py-3 font-medium', isActive('/owner/loyalty') ? 'bg-white text-court-950 shadow-sm' : 'text-court-100 hover:bg-white/10 hover:text-white']">Loyalty</Link>
        ...
        <aside class="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col overflow-y-auto bg-[linear-gradient(180deg,#082f49_0%,#075985_54%,#082f49_100%)] px-4 py-6 text-white lg:flex">
            ...
            <nav v-if="isEmailVerified" class="owner-sidebar-navigation mt-6 space-y-1.5" aria-label="Owner navigation">
                ...
                <template v-if="!isWorkspaceRestricted">
                    ...
                    <Link v-if="abilities.manage_bookings" href="/owner/court-closures" :class="['flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-medium', isActive('/owner/court-closures') ? 'bg-white text-court-900 shadow-sm' : 'text-court-50 hover:bg-white/10']"><svg viewBox="0 0 24 24" class="size-5" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M12 3 2.8 19h18.4L12 3Zm0 5v5m0 3h.01" /></svg>Emergency closures</Link>
                    <Link v-if="organization?.role === 'owner'" href="/owner/team" :class="['flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-medium', isActive('/owner/team') ? 'bg-white text-court-900 shadow-sm' : 'text-court-50 hover:bg-white/10']"><svg viewBox="0 0 24 24" class="size-5" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M16 20v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2M9 10a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm8-3v6m-3-3h6" /></svg>Team & staff</Link>
                    ...
                    <Link v-if="abilities.manage_inventory" href="/owner/promotions" :class="['flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-medium', isActive('/owner/promotions') ? 'bg-white text-court-900 shadow-sm' : 'text-court-50 hover:bg-white/10']"><svg viewBox="0 0 24 24" class="size-5" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M4 12 12 4h7v7l-8 8-7-7Zm11-4h.01M8 13l3 3" /></svg>Promotions</Link>
                    <Link v-if="abilities.manage_loyalty" href="/owner/loyalty" :class="['flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-medium', isActive('/owner/loyalty') ? 'bg-white text-court-900 shadow-sm' : 'text-court-50 hover:bg-white/10']"><svg viewBox="0 0 24 24" class="size-5" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="m12 3 2.8 5.7 6.2.9-4.5 4.4 1.1 6.2-5.6-2.9-5.6 2.9 1.1-6.2L3 9.6l6.2-.9L12 3Z" /></svg>Loyalty</Link>
                    <Link v-if="abilities.manage_inventory" href="/owner/booking-links" :class="['flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-medium', isActive('/owner/booking-links') ? 'bg-white text-court-900 shadow-sm' : 'text-court-50 hover:bg-white/10']"><svg viewBox="0 0 24 24" class="size-5" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.5.5l2-2a5 5 0 0 0-7-7l-1.2 1.2M14 11a5 5 0 0 0-7.5-.5l-2 2a5 5 0 0 0 7 7l1.2-1.2" /></svg>Booking Links</Link>
