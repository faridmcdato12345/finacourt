...
<template>
    <div class="min-h-screen bg-[#f6f8f5]">
        <header class="sticky top-0 z-40 border-b border-court-900 bg-court-950 text-white lg:hidden">
            ...
            <nav class="scrollbar-none flex gap-1 overflow-x-auto px-3 pb-3 text-sm" aria-label="Mobile owner navigation">
                ...
                <Link v-if="organization?.role === 'owner'" href="/owner/directory-claims" :class="['whitespace-nowrap rounded-lg px-3 py-2', isActive('/owner/directory-claims') ? 'bg-white text-court-950' : 'text-court-100']">Claim venue</Link>
        ...
        <aside class="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col overflow-y-auto bg-[linear-gradient(180deg,#063c2a_0%,#075438_54%,#073b2b_100%)] px-4 py-6 text-white lg:flex">
            ...
            <nav class="mt-6 space-y-1.5" aria-label="Owner navigation">
            ...
            <div class="mt-auto pt-8">
                ...
                <div class="flex items-center gap-3 border-t border-white/15 px-2 pt-4"><span class="grid size-9 shrink-0 place-items-center rounded-full bg-court-200 font-semibold text-court-950">{{ user?.name?.slice(0, 1) }}</span><div class="min-w-0 flex-1"><p class="truncate text-sm font-medium">{{ user?.name }}</p><p class="truncate text-xs text-court-200">{{ user?.email }}</p></div><LogoutForm class="rounded-lg p-2 text-sm font-semibold text-court-200 hover:bg-white/10 hover:text-white disabled:cursor-wait disabled:opacity-60" aria-label="Sign out" /></div>
