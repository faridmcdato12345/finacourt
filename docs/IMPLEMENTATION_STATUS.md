...
Phase 18 private owner-invitation update: 2026-08-31
Google Business Profile connection-foundation update: 2026-08-31
...
Phase 15 is complete for the approved progressive scope. Stage A adds the owner Visibility Center, documented deterministic profile score, server-issued QR assets and Phase 13 attribution, safe Google Maps directions, SEO checklist, and booking-link status. Stage B adds a provider-neutral, explicit-confirmation Place persistence boundary while keeping the live Places picker disabled without approved credentials. The Stage C connection foundation now adds optional owner-authorized OAuth, encrypted tokens, tenant-bound state, read-only accessible-profile discovery, explainable matching, explicit confirmation, reconnect/disconnect, duplicate prevention, and audit history. Google profile creation, verification, mutation, ongoing sync, and performance import remain intentionally disabled; live use still requires Google project approval and production OAuth credentials.
...
Phase 18 is complete. It adds a separate non-transactional venue directory with row-level provenance and rights attestation, public unclaimed/closed/claimed states, corrections/removal reporting, admin-issued private owner invitations, independent ownership proof, a safety hold, audited conversion into an unpublished tenant venue, a separate marketplace review/revocation gate, and privacy-minimized pre-claim profile analytics. Public pages no longer solicit ownership requests. No fake owner, password, organization, membership, court, price, photo, review, availability, booking, or verification badge is created.
...
| 15 — Visibility Center and Google/search visibility | **Completed for approved Stage A/B and Stage C connection foundation** | Owner readiness/SEO checks, deterministic scoring, stable QR destinations/attribution, directions, Place ID fields, no-Google fallback, encrypted owner-authorized Business Profile access, read-only managed-location discovery, matching/confirmation, reconnect/disconnect, and audit history are implemented. Live Places, Google profile creation/verification/mutation/sync, and performance import remain disabled. |
...
| 18 — Unclaimed venue directory / claim flow | **Completed and claim-hardened** | A separate provenance-controlled directory, transparent public restrictions, source verification, correction/removal reports, admin-issued hashed/expiring/single-use owner links, verified-account owner requests, independent venue-contact/offline proof, rate limits, a 24-hour safety hold, audited tenant assignment, a separate marketplace review/revocation gate, and aggregate pre-claim profile activity are implemented. Public claim entry is removed; no unverified public seed data or fake credentials are created. |
...
**Models:** `User`, `Organization`, `Membership`, `Venue`, `VenuePhoto`, `VenueReview`, `CourtResource`, `Sport`, `Amenity`, `OperatingHour`, `Booking`, `BookingAttribution`, `Payment`, `PaymentTransition`, `OwnerPayoutProfile`, `OwnerSettlementEntry`, `OwnerPayout`, `OwnerPayoutEvent`, `Promotion`, `PromotionSlot`, `AnalyticsEvent`, `PsgcLocation`, `MarketingPreference`, `ReactivationCampaign`, `ReactivationCampaignRecipient`, `VisibilityLink`, `GoogleBusinessProfileConnection`, `GoogleBusinessProfileOAuthState`, `GoogleBusinessProfileAudit`, `SalesPartnerProfile`, `SalesLead`, `SalesPartnerAttribution`, `CommissionRule`, `CommissionEntry`, `PartnerPayout`, `PartnerAuditEvent`, `VenueDirectoryListing`, `VenueDirectoryHour`, `VenueClaimInvitation`, `VenueClaimRequest`, `VenueDirectoryReport`, and `VenueDirectoryAudit`.
...
- Visibility: `VisibilityScore`, `VenuePublicUrl`, `VisibilityLinkManager`, `GoogleDirections`, `ConfirmVenuePlace`, provider-neutral Places boundaries, and the optional Google Business Profile OAuth/client/discovery/matching/connection services.
...
- Venue directory: `VenueDirectoryManager`, `VenueClaimInvitationService`, `VenueClaimWorkflow`, and `VenueDirectoryAudit`.
...
- Public pages expose no ownership-request button or direct claim route. Only platform admins can create the private link; token secrets are hashed, expiring, listing-bound, single-use, replaceable, revocable, and stopped when a listing is edited or hidden.
- Only a persisted tenant owner following a valid private invitation can request a claim. Staff are denied, browser organization IDs are ignored, and a locked/unique active key prevents duplicate pending claims.
...
- The venue edit page now adds a separate Google-readiness checklist and read-only URL generated from the existing named public route. It never asks owners to type a second booking URL.
- Optional Business Profile OAuth requests only `business.manage`, stores encrypted tokens, validates a hashed ten-minute user/venue/tenant state, discovers only locations the owner-authorized account can manage, and requires candidate confirmation. Exact, likely, ambiguous, no-match, duplicate, revoked, disabled, and outage states are handled without changing venue-save behavior.
- Google profile creation, verification, profile edits, automatic field/booking-link sync, review/photo/post management, and performance import remain intentionally disabled.
...
- Platform admins create a private owner invitation only for a checked, published, claimable listing. The 256-bit secret is shown once and stored only as a SHA-256 hash; the link expires, is single-use, can be replaced/revoked, and is invalidated by listing edits/hiding. The platform sends it manually through a contact channel it has checked; no email/Messenger delivery API was invented.
- Claim requests derive the organization from `TenantContext`, require the active `owner` membership, a verified account email, and a valid private invitation, and are rate limited. Claimant-supplied contact/evidence and public reporter contact email are encrypted at rest but are explicitly non-authoritative.
...
- Private-invitation focused suite: **18 tests passed, 195 assertions**, covering no public claim action, platform-only link issue, no plaintext secret in the database-backed session, hashed secrets, expiry, replacement, revocation, listing-state invalidation, sign-in/registration return paths, staff/tenant boundaries, one-time use, proof gates, and existing conversion safety.
- Full Docker backend suite after the private-invitation update: **268 tests passed, 2,427 assertions**.
- Repository-wide Pint check passed for **420 files**; frontend component tests passed (**6 tests**), and the production Vite build passed with **3,064 modules transformed**.
- Migration `2026_08_31_000033_create_venue_claim_invitations_table.php` was fresh-applied, rolled back, and reapplied against isolated Docker MySQL, then applied to the running Docker application database.
...
- No queue worker, Redis service, external directory/identity provider, scraper, public seed record, fake owner credential, password, or API key was added. `DIRECTORY_CLAIM_INVITATION_HOURS` is the only new setting and defaults to seven days.
...
- Google Business Profile connection-foundation suite: **22 tests passed, 171 assertions**, covering Google-independent venue creation, environment-aware generated URLs, encrypted and browser-hidden tokens, hashed/expiring tenant-bound OAuth state, tamper/replay protection, queued post-OAuth discovery, transient-failure backoff, retry without repeat consent, stale-job and cross-tenant protection, exact/ambiguous/no-match discovery, explicit confirmation, duplicate prevention, API failure containment, replacement-safe reconnect, token-free platform support visibility, disconnect/revoke behavior, current Account Management/Business Information request contracts, and rate-limit sanitization.
- Current repository-wide Docker suite after queued discovery: **290 tests passed, 2,599 assertions**. The combined Google Business Profile and Visibility Center regression run passed **29 tests, 235 assertions**; frontend tests passed **6 tests**, and the production build completed with **3,065 modules transformed**.
- Migration `2026_08_31_000034_create_google_business_profile_tables.php` adds reversible tenant-bound connection, one-time OAuth state, and audit tables. Migration `2026_08_31_000035_add_discovery_generation_to_google_business_profile_connections.php` adds an opaque current-attempt marker that prevents stale queue jobs from overwriting newer owner authorization. The existing database-backed `queue` service handles discovery; no new Docker service or host dependency is required.
- Live profile creation, verification, write/sync, and performance APIs were not implemented. See [Google Business Profile connection](integrations/google-business-profile.md).
...
Phase 11 adds one reversible migration: `2026_08_24_000018_add_demand_dimensions_to_analytics_events.php`. Phase 12 adds one reversible migration: `2026_08_24_000019_extend_promotions_for_engine_v2.php`. Phase 13 adds one reversible migration: `2026_08_24_000020_create_booking_attributions_table.php`. Phase 14 adds one reversible migration: `2026_08_24_000021_create_customer_reactivation_tables.php`. Phase 15 adds one reversible migration: `2026_08_24_000022_create_visibility_center_tables.php`. Phase 16 adds one reversible migration: `2026_08_24_000023_create_sales_partner_tables.php`. Phase 17 adds one reversible migration: `2026_08_24_000024_create_growth_recommendation_states_table.php`. Phase 18 adds `2026_08_25_000025_create_venue_directory_tables.php`; its PSGC directory-location enhancement adds `2026_08_26_000026_add_psgc_codes_to_venue_directory_listings.php`, claim hardening adds `2026_08_26_000027_harden_venue_claim_verification.php`, and the private owner entry point adds `2026_08_31_000033_create_venue_claim_invitations_table.php`. All are reversible. None requires Redis or a new runtime service. Venue email challenges use the existing Laravel mail transport. The private-link lifetime uses `DIRECTORY_CLAIM_INVITATION_HOURS`. Phase 15 adds `endroid/qr-code` for local SVG generation; production-build verification refreshes ignored/generated `public/build` output.

The later Google Business Profile connection foundation adds reversible migration `2026_08_31_000034_create_google_business_profile_tables.php`; queued discovery adds `2026_08_31_000035_add_discovery_generation_to_google_business_profile_connections.php`. It requires no Redis or new runtime service: the existing database worker processes owner-initiated read-only discovery with bounded backoff, while future write/sync work remains deferred.
