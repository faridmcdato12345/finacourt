...
### Court-owner acquisition and transaction pricing

`/for-court-owners` is the public acquisition page for venue operators, while `/pricing` explains the current transaction-based model. Both are server-rendered, indexable Blade pages and are included in the sitemap. The owner page explains the existing discovery, reservations, promotion attribution, and analytics capabilities; its marketplace footprint counters query only currently published venues with active inventory. It does not publish seeded ROI, revenue, visitor, or demand claims as proof.

The pricing page reads the same effective `PlatformServiceFeeRule` used by player checkout, so it never publishes a separate display-only percentage. It shows the owner-set court price, FinACourt service fee, and player total as separate amounts. When no fee rule is active, the page shows a safe no-fee state instead of inventing a rate. The application does not currently have an owner registration charge or recurring owner-subscription billing flow; this describes the current model and is not a permanent free-pricing promise. Verified online court-price earnings can enter the owner settlement balance, subject to refunds, reversals, disputes, provider charges, approved adjustments, and the configured payout delay. Pay-at-venue money is collected directly by the venue and never enters that online balance. Set `OWNER_SALES_EMAIL` to a monitored pricing contact before deployment.
...
Player-initiated online refund requests are full refunds and must be submitted at least `PLAYER_REFUND_CUTOFF_HOURS` before the booked start time (24 hours by default). A player who pays after that normal deadline receives a short `PLAYER_REFUND_GRACE_MINUTES` cooling-off period (15 minutes by default), which never extends beyond game time. The server enforces both deadlines even when a form request is crafted manually. Venue emergency closures use their separate platform-approved refund workflow and remain eligible regardless of the player cutoff.
