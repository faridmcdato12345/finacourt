    ...
    public function evaluate(GrowthRecommendationContext $context): Collection
    {
        ...
        return $context->marketplaceVenues()->map(function (Venue $venue) use ($context, $minimum): ?GrowthRecommendation {
            ...
            return new GrowthRecommendation(
                ...
                title: "{$market['unfulfilled_searches']} nearby {$sport->name} searches went unfulfilled",
                explanation: "In {$venue->city}, {$market['unfulfilled_searches']} searches went unfulfilled during the last {$context->lookbackDays} days: {$market['no_results']} found no matching inventory and {$market['no_availability']} found venues but no bookable time. Counts are shown only after the demand privacy threshold is met.",
                evidence: [
                ...
                actionLabel: $slot === null ? 'Review venue availability' : 'Promote an open slot',
                actionUrl: $action,
